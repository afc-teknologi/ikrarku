...
function Sidebar({ role, view, setView, open, logout, currentAccount }: { role: Role; view: View; setView: (view: View) => void; open: boolean; logout: () => void; currentAccount?: AuthAccount }) {
  ...
  return <aside className={`sidebar ${open ? '' : 'hidden'}`}><Brand /><div className="workspace-pill-wrap"><button className="workspace-pill" onClick={()=>setPillOpen(value=>!value)}><div className="avatar-mini">{(currentAccount?.name || 'IK').slice(0,2).toUpperCase()}</div><div><strong>{currentAccount?.name || (role === 'User' ? 'Customer Workspace' : role === 'Editor' ? 'Web Designer Studio' : role === 'Customer Service' ? 'Support Desk' : 'Administrator Console')}</strong><span>{currentAccount?.email || `${displayRole(role)} workspace`}</span></div>{pillOpen?<ChevronUp size={14} />:<ChevronDown size={14} />}</button>{pillOpen&&<div className="workspace-pill-menu"><button onClick={()=>{setPillOpen(false);setView('settings')}}><Settings size={15}/>Account Settings</button><button className="danger" onClick={()=>{setPillOpen(false);logout()}}><LogOut size={15}/>Sign Out</button></div>}</div><nav>{menus.map(([target, Icon, label]) => <button className={view === target ? 'active' : ''} onClick={() => setView(target)} key={target}><Icon size={18} /><span>{label}</span></button>)}</nav><div className="sidebar-bottom">{role === 'User' && <div className="upgrade-card"><Crown size={18} /><strong>ikrarku Signature</strong><span>Buka template premium dan analytics lengkap.</span><button>Explore plan</button></div>}<button className="support-link danger" onClick={logout}><LogOut size={16} /> Sign out</button></div></aside>
}
...
function Stat({ icon: Icon, label, value, note, onClick }: { icon: typeof Users; label: string; value: string; note: string; onClick?: () => void }) {
  return <div className={`stat-card ${onClick?'clickable':''}`} onClick={onClick} role={onClick?'button':undefined} tabIndex={onClick?0:undefined} onKeyDown={onClick?(event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();onClick()}}):undefined}><div className="stat-icon"><Icon size={18} /></div><div><span>{label}</span><strong>{value}</strong><small>{note}</small></div>{onClick&&<ArrowRight className="stat-go" size={16}/>}</div>
}
